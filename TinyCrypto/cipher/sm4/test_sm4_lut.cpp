#ifdef TINY_CRYPTO_TEST

#include "sm4_lut.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>

using namespace tc;

static uint8_t user_key[16] = {
    0x04, 0xb5, 0xf0, 0x47, 0x03, 0xe2, 0x02, 0x5f,
    0x5d, 0x08, 0x46, 0xc8, 0x0a, 0x68, 0x19, 0xa0,
};

static uint8_t pt[] = {
    ,
};

static uint8_t ct[] = {
    ,
};

int main()
{
    Sm4LutCTX sm4ctx;
    uint8_t   buffer[sizeof(pt)];
    sm4_lut_enc_key_init(&sm4ctx, user_key);
    sm4_lut_enc_blocks(&sm4ctx, buffer, pt, sizeof(pt) / SM4_BLOCK_SIZE);
    if (memcmp(buffer, ct, sizeof(ct)) != 0)
    {
        fprintf(stderr, "err in sm4 lut enc, file: %s, line: %d\n", __FILE__,
                __LINE__);
        exit(-1);
    }
    sm4_lut_dec_key_init(&sm4ctx, user_key);
    sm4_lut_dec_blocks(&sm4ctx, buffer, ct, sizeof(ct) / SM4_BLOCK_SIZE);
    if (memcmp(buffer, pt, sizeof(pt)) != 0)
    {
        fprintf(stderr, "err in sm4 lut dec, file: %s, line: %d\n", __FILE__,
                __LINE__);
        exit(-1);
    }
    puts("sm4 lut test OK!");
    return 0;
}
#endif