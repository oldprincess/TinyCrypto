...
/**
 * https://www.rfc-editor.org/rfc/rfc3174#section-7.2
 */
 ...
 *
 */
static int SHA1Input(SHA1Context*, const uint8_t*, size_t lengt);
...
{
    if (SHA1Input(&ctx->ctx, in, inl) != shaSuccess)
    {
        return -1;
    }
    return 0;
...
{
    if (SHA1Result(&ctx->ctx, digest) != shaSuccess)
    {
        return -1;
    }
    return 0;
