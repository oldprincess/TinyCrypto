...
int main()
{
    ...
    // ************** SHA224 ******************
    // ****************************************
    Sha224StandardCTX ctx224;
    sha224_standard_init(&ctx224);
    uint8_t out224[SHA224_DIGEST_SIZE];
    for (int i = 0; i < 5; i++)
    {
        sha224_standard_reset(&ctx224);
        if (sha224_standard_update(&ctx224, msg[i], msg_size[i]))
        {
            fprintf(stderr, "err in sha224 standard, file: %s, line: %d\n",
                    __FILE__, __LINE__);
            exit(-1);
        }
        sha224_standard_final(&ctx224, out224);
    ...
    // ************** SHA256 ******************
    // ****************************************
    Sha256StandardCTX ctx256;
    sha256_standard_init(&ctx256);
    uint8_t out256[SHA256_DIGEST_SIZE];
    for (int i = 0; i < 5; i++)
    {
        sha256_standard_reset(&ctx256);
        if (sha256_standard_update(&ctx256, msg[i], msg_size[i]))
        {
            fprintf(stderr, "err in sha256 standard, file: %s, line: %d\n",
                    __FILE__, __LINE__);
            exit(-1);
        }
        sha256_standard_final(&ctx256, out256);
    ...
    // ************** SHA384 ******************
    // ****************************************
    Sha384StandardCTX ctx384;
    sha384_standard_init(&ctx384);
    uint8_t out384[SHA384_DIGEST_SIZE];
    for (int i = 0; i < 5; i++)
    {
        sha384_standard_reset(&ctx384);
        if (sha384_standard_update(&ctx384, msg[i], msg_size[i]))
        {
            fprintf(stderr, "err in sha384 standard, file: %s, line: %d\n",
                    __FILE__, __LINE__);
            exit(-1);
        }
        sha384_standard_final(&ctx384, out384);
        if (memcmp(out384, digest384[i], SHA384_DIGEST_SIZE) != 0)
        {
            fprintf(stderr, "err in sha384 standard, file: %s, line: %d\n",
                    __FILE__, __LINE__);
            exit(-1);
    ...
    // ************** SHA512 ******************
    // ****************************************
    Sha512StandardCTX ctx512;
    sha512_standard_init(&ctx512);
    uint8_t out512[SHA512_DIGEST_SIZE];
    for (int i = 0; i < 5; i++)
    {
        sha512_standard_reset(&ctx512);
        if (sha512_standard_update(&ctx512, msg[i], msg_size[i]))
        {
            fprintf(stderr, "err in sha512 standard, file: %s, line: %d\n",
                    __FILE__, __LINE__);
            exit(-1);
        }
        sha512_standard_final(&ctx512, out512);
        if (memcmp(out512, digest512[i], SHA512_DIGEST_SIZE) != 0)
        {
            fprintf(stderr, "err in sha512 standard, file: %s, line: %d\n",
                    __FILE__, __LINE__);
            exit(-1);
    ...
    puts("sha2 standard (sha224/256/384/512) test ok!");
    return 0;
}
