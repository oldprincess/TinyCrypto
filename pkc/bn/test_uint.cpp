...
#define uint256_add_carry(a, b, c)         uint_add_carry(8, a, b, c, 0)
#define uint256_sub_borrow(a, b, c)        uint_sub_borrow(8, a, b, c, 0)
#define uint256_mul(a, b, c)               uint_mul(8, a, b, c)
#define uint256_sqr(a, b)                  uint_sqr(8, a, b)
#define uint256_add_carry_uint32(a, b, c)  uint_add_carry_uint32(8, a, b, c)
#define uint256_sub_borrow_uint32(a, b, c) uint_sub_borrow_uint32(8, a, b, c)
#define uint256_mul_carry_uint32(a, b, c)  uint_mul_carry_uint32(8, a, b, c)
#define uint256_cmp(a, b)                  uint_cmp(8, a, b)
#define uint256_cmp_uint32(a, b)           uint_cmp_uint32(8, a, b)
#define uint256_equal(a, b)                uint_equal(8, a, b)
#define uint256_equal_zero(a)              uint_equal_zero(8, a)
#define uint256_equal_one(a)               uint_equal_one(8, a)
#define uint256_cpy(a, b)                  uint_cpy(8, a, b)
#define uint256_set_zero(a)                uint_set_zero(8, a)
#define uint256_set_one(a)                 uint_set_one(8, a)
#define uint256_set_uint32(a, b)           uint_set_uint32(8, a, b)
#define uint256_from_bytes(a, b)           uint_from_bytes(8, a, b)
#define uint256_to_bytes(a, b)             uint_to_bytes(8, a, b)
#define uint256_bittest(a, b)              uint_bittest(8, a, b)
...
static bool test_uint_div()
{
    srand(time(NULL));
    uint32_t a[8] = {0}, b[8] = {0}, c[16];
    uint32_t x[16], y[16], r1[16], r2[16];
    for (int t = 0; t < 100; t++)
    {
        for (int i = 0; i < rand() % 7 + 1; i++)
        {
            a[i] = rand() << 24 | rand() << 16 | rand() << 8 | rand();
            b[i] = rand() << 24 | rand() << 16 | rand() << 8 | rand();
        }
        if (rand() % 4 == 0)
        {
            uint_set_uint32(8, b, rand() + 1);
        }
        uint_mul(8, c, a, b);
        uint_cast(x, 16, a, 8);
        uint_cast(y, 16, b, 8);
        if (uint_div(16, x, r1, c, x))
        {
            printf("%s, %d\n", __FUNCTION__, __LINE__);
            return false;
        }
        if (uint_div(16, y, r2, c, y))
        {
            printf("%s, %d\n", __FUNCTION__, __LINE__);
            return false;
        }
        if (!uint_equal(8, x, b) || !uint_equal(8, y, a) ||
            !uint_equal_zero(16, r1) || !uint_equal_zero(16, r2))
        {
            printf("%s, %d\n", __FUNCTION__, __LINE__);
            return false;
        }
...
static bool test_uint_str()
{
    ...
    for (int t = 0; t < 100; t++)
    {
        ...
        {
            size_t len = uint_to_str_max_outl(8, radix);
            if (len > sizeof(str))
            {
                printf("%s, %d\n", __FUNCTION__, __LINE__);
                return false;
            }
            if (uint_to_str(8, str, a, radix))
            {
                printf("%s, %d\n", __FUNCTION__, __LINE__);
                return false;
            }
            ...
            if (uint_from_str(8, b, str, strlen(str), radix))
            {
                printf("%s, %d\n", __FUNCTION__, __LINE__);
                return false;
            }
