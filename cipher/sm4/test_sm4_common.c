...
int main()
{
    ...
    sm4_common_enc_key_init(&sm4ctx, user_key);
    sm4_common_enc_blocks(&sm4ctx, buffer, pt, sizeof(pt) / SM4_BLOCK_SIZE);
    if (memcmp(buffer, ct, sizeof(ct)) != 0)
    {
        fprintf(stderr, "err in sm4 common enc, file: %s, line: %d\n", __FILE__,
                __LINE__);
        exit(-1);
    }
    sm4_common_dec_key_init(&sm4ctx, user_key);
    sm4_common_dec_blocks(&sm4ctx, buffer, ct, sizeof(ct) / SM4_BLOCK_SIZE);
    if (memcmp(buffer, pt, sizeof(pt)) != 0)
    {
        fprintf(stderr, "err in sm4 common dec, file: %s, line: %d\n", __FILE__,
                __LINE__);
        exit(-1);
    }
    puts("sm4 common test OK!");
    return 0;
}