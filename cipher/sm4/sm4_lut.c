...
#pragma region "Sm4Lut Params And Define"
...
#pragma region "Sm4Lut Core Func"
 ...
 */
static void sm4_lut_compute_block(const uint32_t round_key[32],
                                  uint8_t        out[16],
                                  const uint8_t  in[16])
{
    uint32_t x[4];
 ...
 */
static void sm4_lut_compute_block_x4(const uint32_t round_key[32],
                                     uint8_t        out[64],
                                     const uint8_t  in[64])
{
...
static void sm4_compute_blocks(const uint32_t round_key[32],
                               uint8_t*       out,
                               const uint8_t* in,
                               size_t         block_num)
{
    while (block_num >= 4)
    {
        sm4_lut_compute_block_x4(round_key, out, in);
        out += 4 * 16, in += 4 * 16;
        block_num -= 4;
    }
    while (block_num)
    {
        sm4_lut_compute_block(round_key, out, in);
...
{
    sm4_lut_compute_block(ctx->round_key, ciphertext, plaintext);
}
...
{
    sm4_lut_compute_block(ctx->round_key, plaintext, ciphertext);
}
...
{
    sm4_lut_compute_block_x4(ctx->round_key, ciphertext, plaintext);
}
...
{
    sm4_lut_compute_block_x4(ctx->round_key, plaintext, ciphertext);
}
