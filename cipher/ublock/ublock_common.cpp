...
static inline uint64_t lut_u64_epi4(uint64_t n, const uint8_t T[16])
{
    uint64_t ret;
    ret = (uint64_t)T[(n >> 60) & 0xF] << 60;
    ret |= (uint64_t)T[(n >> 56) & 0xF] << 56;
    ret |= (uint64_t)T[(n >> 52) & 0xF] << 52;
    ret |= (uint64_t)T[(n >> 48) & 0xF] << 48;
    ret |= (uint64_t)T[(n >> 44) & 0xF] << 44;
    ret |= (uint64_t)T[(n >> 40) & 0xF] << 40;
    ret |= (uint64_t)T[(n >> 36) & 0xF] << 36;
    ret |= (uint64_t)T[(n >> 32) & 0xF] << 32;
    ret |= (uint64_t)T[(n >> 28) & 0xF] << 28;
    ret |= (uint64_t)T[(n >> 24) & 0xF] << 24;
    ret |= (uint64_t)T[(n >> 20) & 0xF] << 20;
    ret |= (uint64_t)T[(n >> 16) & 0xF] << 16;
    ret |= (uint64_t)T[(n >> 12) & 0xF] << 12;
    ret |= (uint64_t)T[(n >> 8) & 0xF] << 8;
    ret |= (uint64_t)T[(n >> 4) & 0xF] << 4;
    ret |= (uint64_t)T[(n >> 0) & 0xF] << 0;
    return ret;
}
static inline uint32_t Sk_u32(uint32_t n)
{
    uint32_t ret;
    ret = (uint32_t)SBOX[(n >> 28) & 0xF] << 28;
    ret |= (uint32_t)SBOX[(n >> 24) & 0xF] << 24;
    ret |= (uint32_t)SBOX[(n >> 20) & 0xF] << 20;
    ret |= (uint32_t)SBOX[(n >> 16) & 0xF] << 16;
    ret |= (uint32_t)SBOX[(n >> 12) & 0xF] << 12;
    ret |= (uint32_t)SBOX[(n >> 8) & 0xF] << 8;
    ret |= (uint32_t)SBOX[(n >> 4) & 0xF] << 4;
    ret |= (uint32_t)SBOX[(n >> 0) & 0xF] << 0;
    return ret;
}
static inline uint64_t Sk_u64(uint64_t n)
{
    return lut_u64_epi4(n, SBOX);
}
static inline uint64_t SIk_u64(uint64_t n)
{
    return lut_u64_epi4(n, SBOXI);
}
static inline uint32_t Tk_u32(uint32_t n)
{
    uint32_t ret;
...
static inline uint64_t Tk_u64(uint64_t n)
{
    uint64_t ret;
    ret = (uint64_t)MUL2T[(n >> 60) & 0xF] << 60;
    ret |= (uint64_t)MUL2T[(n >> 56) & 0xF] << 56;
    ret |= (uint64_t)MUL2T[(n >> 52) & 0xF] << 52;
    ret |= (uint64_t)MUL2T[(n >> 48) & 0xF] << 48;
    ret |= (uint64_t)MUL2T[(n >> 44) & 0xF] << 44;
    ret |= (uint64_t)MUL2T[(n >> 40) & 0xF] << 40;
    ret |= (uint64_t)MUL2T[(n >> 36) & 0xF] << 36;
    ret |= (uint64_t)MUL2T[(n >> 32) & 0xF] << 32;
    ret |= (uint64_t)MUL2T[(n >> 28) & 0xF] << 28;
    ret |= (uint64_t)MUL2T[(n >> 24) & 0xF] << 24;
    ret |= (uint64_t)MUL2T[(n >> 20) & 0xF] << 20;
    ret |= (uint64_t)MUL2T[(n >> 16) & 0xF] << 16;
    ret |= (uint64_t)MUL2T[(n >> 12) & 0xF] << 12;
    ret |= (uint64_t)MUL2T[(n >> 8) & 0xF] << 8;
    ret |= (uint64_t)MUL2T[(n >> 4) & 0xF] << 4;
    ret |= (uint64_t)MUL2T[(n >> 0) & 0xF] << 0;
    return ret;
}
static inline uint64_t shuffle_u64_epi4(uint64_t n, const int T[16])
{
    uint64_t ret;
...
static inline uint64_t shuffle_u64_epi8(uint64_t n, const int T[8])
{
    uint64_t ret;
...
static inline void shuffle_u128_epi8(uint64_t n[2], const int T[16])
{
...
static inline uint64_t rotl_u64_epi32(uint64_t n, int s)
{
    uint32_t n0 = (uint32_t)(n >> 32);
    uint32_t n1 = (uint32_t)(n & UINT32_MAX);
    uint32_t r0 = (n0 << s) | (n0 >> (32 - s));
    uint32_t r1 = (n1 << s) | (n1 >> (32 - s));
    return ((uint64_t)r0 << 32) | (uint64_t)r1;
}
static inline void PK1_u64(uint32_t *K0, uint32_t *K1)
{
    uint64_t K0K1 = (uint64_t)(*K0) << 32 | (uint64_t)(*K1);
    uint64_t ret  = shuffle_u64_epi4(K0K1, PK1T);
    *K0           = (uint32_t)(ret >> 32);
    *K1           = (uint32_t)(ret & UINT32_MAX);
}
static inline void PK2_u128(uint64_t *K0, uint64_t *K1)
{
    *K0 = shuffle_u64_epi4(*K0, PK2T_H);
    *K1 = shuffle_u64_epi4(*K1, PK2T_L);
}
static inline void PK3_u128(uint64_t *K0, uint64_t *K1)
{
    *K0 = shuffle_u64_epi4(*K0, PK3T_H);
    *K1 = shuffle_u64_epi4(*K1, PK3T_L);
}
static inline uint64_t PL128_u64(uint64_t n)
{
    return shuffle_u64_epi8(n, PL128T);
}
static inline uint64_t PR128_u64(uint64_t n)
{
    return shuffle_u64_epi8(n, PR128T);
}
static inline uint64_t PL128I_u64(uint64_t n)
{
    return shuffle_u64_epi8(n, PL128IT);
}
static inline uint64_t PR128I_u64(uint64_t n)
{
    return shuffle_u64_epi8(n, PR128IT);
}
static inline void PL256_u128(uint64_t n[2])
{
    shuffle_u128_epi8(n, PL256T);
}
static inline void PR256_u128(uint64_t n[2])
{
    shuffle_u128_epi8(n, PR256T);
}
static inline void PL256I_u128(uint64_t n[2])
{
    shuffle_u128_epi8(n, PL256IT);
}
static inline void PR256I_u128(uint64_t n[2])
{
...
static void ublock128128_key_init(uint64_t      round_key[17][2],
                                  const uint8_t user_key[16])
{
    ...
    // RK0 = K[0:127]
    round_key[0][0] = ((uint64_t)K[0] << 32) | K[1];
    round_key[0][1] = ((uint64_t)K[2] << 32) | K[3];
    for (int i = 1; i <= 16; i++)
    {
        // K0||K1||K2||K3 <- K
        ...
        // RKi = K[0:127]
        round_key[i][0] = ((uint64_t)K[0] << 32) | K[1];
        round_key[i][1] = ((uint64_t)K[2] << 32) | K[3];
    }
}

static void ublock128256_key_init(uint64_t      round_key[25][2],
...
static void ublock256256_key_init(uint64_t      round_key[25][4],
                                  const uint8_t user_key[32])
{
    uint64_t K[4];
    K[0] = MEM_LOAD64BE(user_key + 0);
...
static void ublock128128_enc_block(const uint64_t round_key[17][2],
                                   uint8_t        out[16],
                                   const uint8_t  in[16])
{
    // X0||X1 <- X
    uint64_t X0 = MEM_LOAD64BE(in + 0);
    uint64_t X1 = MEM_LOAD64BE(in + 8);
    uint64_t RK0, RK1;
    for (int i = 0; i < 16; i++)
    {
    ...
    // Y <- RKr ^ (X0||X1)
    uint64_t Y0 = round_key[16][0] ^ X0;
    uint64_t Y1 = round_key[16][1] ^ X1;
    MEM_STORE64BE(out + 0, Y0);
    MEM_STORE64BE(out + 8, Y1);
}

static void ublock128128_dec_block(const uint64_t round_key[17][2],
                                   uint8_t        out[16],
                                   const uint8_t  in[16])
{
    // Y0||Y1 <- Y
    uint64_t Y0 = MEM_LOAD64BE(in + 0);
    uint64_t Y1 = MEM_LOAD64BE(in + 8);
    uint64_t RK0, RK1;
    for (int i = 16; i >= 1; i--)
    {
        RK0 = round_key[i][0], RK1 = round_key[i][1];
    ...
    // X <- RK0 ^ (Y0||Y1)
    uint64_t X0 = round_key[0][0] ^ Y0;
    uint64_t X1 = round_key[0][1] ^ Y1;
    MEM_STORE64BE(out + 0, X0);
    MEM_STORE64BE(out + 8, X1);
}

static void ublock128256_enc_block(const uint64_t round_key[25][2],
                                   uint8_t        out[16],
                                   const uint8_t  in[16])
{
    // X0||X1 <- X
    ...
    // Y <- RKr ^ (X0||X1)
    uint64_t Y0 = round_key[24][0] ^ X0;
    uint64_t Y1 = round_key[24][1] ^ X1;
    MEM_STORE64BE(out + 0, Y0);
    MEM_STORE64BE(out + 8, Y1);
}

static void ublock128256_dec_block(const uint64_t round_key[25][2],
                                   uint8_t        out[16],
                                   const uint8_t  in[16])
{
    // Y0||Y1 <- Y
...
static void ublock256256_enc_block(const uint64_t round_key[25][4],
                                   uint8_t        out[32],
                                   const uint8_t  in[32])
{
    uint64_t X0[2], X1[2];
...
static void ublock256256_dec_block(const uint64_t round_key[25][4],
                                   uint8_t        out[32],
                                   const uint8_t  in[32])
{
    uint64_t Y0[2], Y1[2];
